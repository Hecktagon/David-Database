...
def main(infile1, infile2, id_col, numeric_col, only_common_genes = True, n_top_annotations = 10, knowledgebase_directory = "knowledgebase", annotation_outfile1 = "outputs/functional_annotation_results1.tsv", annotation_outfile2 = "outputs/functional_annotation_results_2.tsv", shared_outfile = "outputs/shared_annotation_genes.tsv", final_outfile = "outputs/final_comparison_results.tsv"):
    ...
    knowledgebase_directory: Path to DAVID knowledge base directory.
    ...
    print(f"\n____Extracting IDs From Input Files____")
    ...
    print(f"\n____Calculating Functional Annotations For File {infile1.split("/")[-1]}____")
    annotations_df1 = do_functional_annotation(gene_set1, knowledgebase_directory, annotation_outfile1)
    print(f"\n____Calculating Functional Annotations For File {infile2.split("/")[-1]}____")
    annotations_df2 = do_functional_annotation(gene_set2, knowledgebase_directory, annotation_outfile2)

    # find the categories that had the most genes in common between the two annotation files, and extract the genes in common.
    print(f"\n____Finding Annotations With Most Genes In Common____")
    ...
    print(f"\nFinal results written to {final_outfile}")
